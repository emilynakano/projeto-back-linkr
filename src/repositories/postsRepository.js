...
export async function getAllPosts() {
    return db.query(`SELECT posts.id AS "postId", users.id AS "userId", posts.id, posts.content, users."profilePicture", posts.url FROM users JOIN posts ON users.id=posts."posterId" ORDER BY posts."createdAt" DESC LIMIT 20;
    `)
}
